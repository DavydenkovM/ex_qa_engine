defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>ControllerTest do
  ...
  describe "create <%= schema.singular %>" do
    test "renders <%= schema.singular %> when responce is valid", %{conn: conn} do
  ...
  describe "update <%= schema.singular %>" do
    ...
    test "renders <%= schema.singular %> when responce is valid", %{conn: conn, <%= schema.singular %>: %<%= inspect schema.alias %>{id: id} = <%= schema.singular %>} do
    ...
    test "renders errors when responce is invalid", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
